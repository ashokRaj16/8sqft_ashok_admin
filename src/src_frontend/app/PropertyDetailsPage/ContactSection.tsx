...
const ContactSection: React.FC = () => {
  ...
  const handleScheduleVisitClick = (e: React.MouseEvent) => {
    e.preventDefault();
    if (token) {
      // If logged in, call handleWhatsAppClick
      handleWhatsAppClick();
    } else {
      openDialog(); // Open login dialog if not logged in
    }
  ...
  return (
    <>
      ...
      <Button
          ...
          onClick={handleOwnerContactClick} // Check if logged in before opening dialog
        >
          <FaWhatsapp /> Owner Contact
        </Button>

        <Button
          variant="outline"
          className="flex items-center gap-2 bg-primary text-white"
          onClick={handleScheduleVisitClick} // Check if logged in before opening dialog
        >
          <FaWhatsapp /> Schedule Visit
        </Button>
