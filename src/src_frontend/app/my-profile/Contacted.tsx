...
const Contacted: React.FC<BasicProfileProps> = ({
    setActiveSection }) => {
    ...
    useEffect(() => {
        ...
        fetchInterestedProperties(1, true);
    }, [token]);
    const fetchInterestedProperties = async (pageNumber: number, isInitial = false) => {
        try {
            // console.log("Updated token:", token);
            const response = await axios.get(
                "/api/v1/front/profile/contacted_properties",
                {
                    headers: {
            ...
            const contactedProperties = response.data.data.property || [];
            // console.log("API Response:", contactedProperties);

            // const formattedProperties: Property[] = contactedProperties.map(
            ...
            const formattedProperties: Property[] = contactedProperties.map(
                (property: any, index: number) => ({
                    id: property.id ?? index,  // Use index if id is missing
                    property_title: property.property_title || "No Title Provided",
                    property_img_url: property.property_img_url || "/placeholder.jpg", // Default placeholder image
                    landmark: property.landmark || "No Landmark Available",
                    property_rent_buy: property.property_rent_buy || "-",
                    per_sqft_amount: property.per_sqft_amount ?? "-",
                    builtup_area: property.builtup_area ?? "-",
    ...
    const handleShowMore = () => {
        if (page < totalPages) {
            ...
            fetchInterestedProperties(nextPage);
            setPage(nextPage);
        }
    };
    ...
    const filteredProperties = properties.filter((property) => {
        switch (selectedFilter) {
            ...
            case "PG/Hostel":
                return property.property_rent_buy === "SELL";
            case "OpenPlot":
                return property.property_type === "OpenPlot" || property.property_type === "pg";
            case "Projects":
            ...
            {isMobile ? (
                <div className="p-2">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex  flex-row items-center gap-4 mb-4 border-b-[1px] border-black">
                            <button
                            ...
                            <h1 className="text-lg text-[#222222] font-medium align-middle mx-auto pb-2">To whom you contacted</h1>
                        </div>
                    ...
                    {filteredProperties.length === 0 ? (
                        <p className="text-center text-gray-500">Currently, there is no Contacted Properties</p>
                    ) : (
                        <>
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <div onClick={() => moveToDetailsHandler(property.id)} className="flex-1 ">
                                                <h3 className="font-semibold text-sm">
                                                    {property.property_title.length > 15
                                                ...
                                                <p className="text-[#A4A4A4] text-sm">
                                                    ...
                                                    `${property.city_name},${property.locality}`
                                                </p>
                                                ...
                                                {selectedFilter === "Commercial-Rent" &&
                                                    ...
                                                    property.property_type.includes("commercial") && (
                                                        <>
                                                            <p className="font-sm">
                                                                <span className="text-[#A4A4A4]">Expected Rent:</span> ₹{property.rent_amount}
                                                            </p>
                                                ...
                                                {selectedFilter === "OpenPlot" &&
                                                    (property.property_type.includes("openland") ||
                                                        property.property_type.includes("pg")) && (
                                                        <>
                                                            <p className="font-sm">
                                            ...
                                            <div className="flex flex-row sm:flex-row gap-2 mt-1 h-[38px]">
                                                <button
                                                    ...
                                                    ${isClicked ? "bg-primary text-white border-1" : "text-primary border-primary"}`}
                                                >
                                                    Get Owner Details
                                                </button>
                                                {/* Contact Dialog */}
                                                ...
                                                {/* <button
                                                    ...
                                                    className="text-primary border-[2px] border-primary p-1 rounded-md text-sm w-full sm:w-auto"
                                                >
                                                    Remove
                                                </button> */}
                                            </div>
                ...
                <div className="p-2">
                    <h1 className="text-xl font-semibold mb-4 border-b-2 border-black">To whom you contacted</h1>
                    <div className="flex items-center gap-4 pb-4">
                        <label className="text-sm text-gray-700">Currently Available </label>
                        <button type="button" onClick={() => setWhatsappEnabled(!whatsappEnabled)}>
                            <Image
                    ...
                    {/* 
                    <div className="  sticky top-0 z-10  p-4 overflow-x-auto flex gap-1 mb-4 flex-nowrap ">
                        {filterOptions.map((filter) => (
                            <button
                                key={filter}
                                onClick={() => setSelectedFilter(filter)}
                                className={`px-4 py-2 text-sm rounded-full border whitespace-nowrap ${selectedFilter === filter ? "bg-primary text-white" : "bg-gray-200"
                                    }`}
                            >
                                {filter}
                            </button>
                                        ...
                                        <div className="w-1/2 flex flex-col justify-between">
                                            <div onClick={() => moveToDetailsHandler(property.id)} >
                                                <h3 className="font-semibold">
                                                    {property.property_title.length > 16
                                                        ? `${property.property_title.slice(0, 16)}...`
                                                        : property.property_title}
                                                </h3>
                                                <p className="text-[#A4A4A4] text-sm">
                                                    {property.landmark?.length > 18
                                                        ? `${property.landmark.slice(0, 18)}...`
                                                        : property.landmark || "No Landmark Available"}
                                                    {/* `${property.city_name},${property.locality}` */}
                                                </p>
                                                ...
                                                {selectedFilter === "Residential-Rent" && property.property_type?.toLocaleLowerCase() === "residential" && property.property_rent_buy === "RENT" && (
                                                    <>
                                                        <p className="font-sm">
                                                            <span className="text-[#A4A4A4]">Builtup Area:</span> {property.builtup_area}
                                                        </p>
                                            ...
                                            <div className="flex flex-row w-full gap-2 ">
                                                <button
                                                    onClick={handleOwnerContactClick}
                                                    className={` text-sm p-2 rounded-md border-2 
              ${isClicked
                                                            ? "bg-primary text-white border-primary bottom-1"
