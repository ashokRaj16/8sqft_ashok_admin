...
export default function ContactDeveloperSection({ configration, propertyVariety, propertytype }: ContactDeveloperSectionProps) {
  ...
  console.log(configration, 'plotSizes')
  const handleSubmit = async (values: any, { resetForm }: FormikHelpers<any>) => {
    const requestData = {
      ...
      email: values.email,
      property_id: id,
      selected_plot_size: values.selected_plot_size || values.selectedPropVarity || values.selectedUnitName,
    ...
    try {
      const response = await axios.post("/api/v1/front/contact_developer", requestData, {
        headers: {
        ...
        <Form>
          <Card className=" bg-white">
            <CardContent className="flex flex-col gap-5 p-4">
              <h2 className="font-semibold text-base text-[#222222]">Contact Builder</h2>
              ...
              {successMessage &&
                (
                  <p
                    className={`text-center text-sm mt-4 ${successMessage.error ? "text-red" : "text-green"
                      }`}
                  >
