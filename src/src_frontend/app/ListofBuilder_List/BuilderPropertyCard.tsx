...
import BuilderPropertyCardSimilar from "./BuilderPropertyCardSimilar";
...
const Card = ({ property }: any) => {
  const details = [
    ...
    {
      label: property.config_dimenssion ? `${property.config_dimenssion}` : "N/A",
      alt: "Dimension (LxW)",
      ...
      <div className=" max-w-[408px] max-h-[432px] bg-white rounded-2xl shadow-md flex flex-col  overflow-hidden">
        ...
        <div className="w-full h-[303px] p-5 flex justify-center items-center overflow-hidden relative">
          ...
          <div className="flex items-center gap-1 absolute bg-primary p-1 rounded-md top-7 right-7">
            ...
            <label className=" text-white text-[10px]">{property?.unique_view_count} </label>
          </div>
        </div>
        ...
        <div className="w-full h-[35px] px-5 py-1 border-b border-gray-300 flex items-center">
          <h3 className="text-md font-medium text-gray-800 my-3 line-clamp-1">
            {/* {property.property_title.length || "Property Title"} */}

            {property.property_title && property.property_title.length > 35 ? `${property.property_title.slice(0, 35)} ...` : property.property_title || "Property Title"}
          </h3>
        </div>
...
interface BuilderPropertyCardProps {
  setGetTotalCount: (count: number) => void;
}
...
function BuilderPropertyCard({ setGetTotalCount }: BuilderPropertyCardProps) {
  ...
  useEffect(() => {
    const totalCount = data?.data?.totalCounts || 0;
    setGetTotalCount(totalCount);
  }, [data]);
    ...
    return (
      <div className="text-center text-gray-600 mt-10">
        ...
        <p className="text-sm mt-4 text-start font-semibold">
          In the meantime, here are some similar listings you might like:
        </p>

        <BuilderPropertyCardSimilar />
      </div>
    );
  }
    ...
    <Suspense fallback={<div>Loading...</div>}>
      ...
      <div className="lg:space-y-4 lg:w-full  px-1">
        {properties?.map((property: any) => {
          ...
          return (
            <div
            onClick={() => window.open(`/Builder/${property?.title_slug}`, "_blank") }
              key={property.id}
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                <div className="relative w-full lg:w-[270px]">
                  {property?.rera_number && (<div className=" w-full flex items-center justify-center ">
                ...
                <div className="flex flex-col justify-between w-full">
                  <div className="grid grid-cols-2 lg:grid-cols-2 gap-1 lg:gap-4 border border-gray-300 rounded-lg">
                    {features.map((feature, index) => (
                      ...
                      >
                        <img src={feature.icon} alt={feature.alt} className="w-6 lg:w-8" />
                        <div className="flex flex-col text-left">
                          <span className="text-xs lg:text-sm text-gray-70 font-medium capitalize">{feature.label} </span>
                          <span className="text-[10px] lg:text-xs text-[#22222280]">{feature.subLabel} </span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center mt-4 relative">
                    <Link href={`/Builder/${property?.title_slug}`} target="_blank" rel="noopener noreferrer">
      ...
      <p className="text-lg  mt-10 text-start font-semibold">
