...
export const getAllContactedPropertyList = async (whereClause = null, page = 1, limit = 10, sortColumn = 'id',  sortOrder = 'asc' ) => {
    try {
...
export const getAllContactedPropertyCount = async ( whereClause = null, ) => {
    try {
        const propertyJoin = `JOIN tbl_property tp ON 
        JSON_CONTAINS(tpi.property_id, JSON_OBJECT('pid', tp.id))`
