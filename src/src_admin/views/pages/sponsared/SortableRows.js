...
import { FaArrowRight, FaEye, FaGripVertical, FaPencilAlt, FaTrash } from 'react-icons/fa'
...
const SortableRows = ({ item, changeStatusHandler, deleteHandler, viewHandlder, isCategoreisSame = false, index }) => {
  ...
  return (
    <CTableRow ref={setNodeRef} style={style} {...attributes}>
      ...
      :
      <CTableDataCell >
        <FaArrowRight color="dark-grey" style={{ fontWeight: 'normal', opacity: 0.7 }} />
      </CTableDataCell>
    }
      <CTableDataCell>{item.id}</CTableDataCell>
      <CTableDataCell>{item.property_title || '-'}</CTableDataCell>
      <CTableDataCell>{item.property_title || '-'}</CTableDataCell>
      <CTableDataCell>{item?.fname || '-'}</CTableDataCell>
      <CTableDataCell>{item.categories || '-'}</CTableDataCell>
      <CTableDataCell>{item.sequence_no || '-'}</CTableDataCell>
      <CTableDataCell>{formattedDate(item?.sp_publish_date) || '-'}</CTableDataCell>
      <CTableDataCell>
        <CBadge
          onClick={() =>
            changeStatusHandler(item.tps_id, item.sponsared_status === '1' ? '0' : '1')
          }
      ...
      <CTableDataCell>
        <CButton
          size="sm"
          color="primary"
          onClick={() => viewHandlder(item.tps_id)}
          className="me-2 mb-1"
        >
          <FaEye color="white" />
        </CButton>
        ...
        <CButton
          size="sm"
          color="danger"
          onClick={() => deleteHandler(item.tps_id)}
          className="me-2 mb-1"
        >
          <FaTrash color="white" />
        </CButton>
