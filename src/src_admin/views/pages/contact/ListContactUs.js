...
import { deleteContactUs, getContactUs } from '@model/contactsModel'
import Loader from '@util/Loader'
import ContactUsDetailsModal from './ContactUsDetailsModal'
...
const ListContactUs = () => {
  ...
  const [selectedContactRecords, setSelectedContactRecords] = useState([])
  ...
  const handleSelectAll = () => {
    if (selectedContactRecords.length === currentContact.length) {
      setSelectedContactRecords([])
    } else {
      setSelectedContactRecords(currentContact)
    }
  }

  const handleSelectUser = (user) => {
    setSelectedContactRecords((prevSelectedContacts) => {
      if (prevSelectedContacts.includes(contact)) {
        return prevSelectedContacts.filter((selectedContact) => selectedContact !== contact)
      } else {
  ...
  const handleAction = async (action) => {
    ...
    if (action === 'delete') {
      ...
      try {
        const ids = selectedContactRecords.map((data) => data?.book_id)
  ...
  const loadContactUsData = async () => {
    try {
      ...
      console.log(result)
      setCurrentContact(() => result.data.contacts)
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        setLoading(true)
        console.log(id)
        const result = await deleteContactUs(id)
  ...
  return (
    <>
      {loading && <Loader />}
      { modalVisible?.visible && <ContactUsDetailsModal 
      ...
      <CCard className="mb-4">
        <CCardHeader className="d-flex">
          ...
          <CCol className="d-flex justify-content-end">
            {/* <CButton onClick={() => navigate('/contact/add')} color="primary">
              Add New
            </CButton> */}
            {/* <h1>Member Users</h1> */}
          </CCol>
            ...
            <CTableHead color="light">
              <CTableRow>
                <CTableHeaderCell>
                  {/* <CFormCheck /> */}
                  <CFormCheck
                    checked={selectedContactRecords.length === currentContact.length}
                ...
                <CTableHeaderCell onClick={() => requestSort('full_name')}>
                ...
                <CTableHeaderCell onClick={() => requestSort('message')}>
                  Message {getSortIcon('message')}
                </CTableHeaderCell>
                <CTableHeaderCell onClick={() => requestSort('ip_address')}>
              ...
              {currentContact.map((contact, index) => (
                <CTableRow key={index}>
                  <CTableDataCell>
                    {/* <CFormCheck /> */}
                    <CFormCheck
                      checked={selectedContactRecords.includes(contact)}
                      onChange={() => handleSelectUser(contact)}
                    />
                  </CTableDataCell>
                  <CTableDataCell>{contact.id}</CTableDataCell>
                  <CTableDataCell>{`${contact.full_name || '-'} `}</CTableDataCell>
                  ...
                  <CTableDataCell>{contact.ip_address || '-'}</CTableDataCell>
                  
                  <CTableDataCell>{formattedDate(contact?.created_at) || '-'}</CTableDataCell>
