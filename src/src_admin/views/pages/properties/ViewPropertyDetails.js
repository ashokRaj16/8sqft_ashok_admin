...
import {
  ...
  generatePropertyNearbyProeprtyId,
  getPropertyNearbyAllCategory,
  insertGeneratedNearbyProeprtyId,
  updatePropertyImage,
...
const ViewPropertyDetails = () => {
  ...
  const [activeTab, setActiveTab] = useState(activeTabId || 4)
  ...
  const [generatedNearbyPlacesList, setGeneratedNearbyPlacesList] = useState([])

  console.log(generatedNearbyPlacesList, 'place list')
  ...
  console.log(nearbyCategory, "cateee")
  // ### Will work out later
  const generateNearByLocation = async (id) => {
    try {
      setLoading(true)
      
      // const categories = await getNearbyLocationAllCagetories();
      // if(!categories) {
      //   addToast('Unable to fetch categories.');
      //   return;
      // }
      // setNearbyCategory(categories?.data);
      const result = await generatePropertyNearbyProeprtyId(id)
      if (result) {
        const flatPlacesList = Object.values(result?.data)
          .flat() // flatten all category arrays
          .map((item) => ({
        ...
        // console.log(flatPlacesList, 'list')
        setGeneratedNearbyPlacesList(flatPlacesList)
        addToast('success', result.message)
      }
    } catch (error) {
      addToast('error', error.message)
  ...
  const saveGeneratedLocationHandler = async (e) => {
    try {
      if (!generatedNearbyPlacesList.length) {
        addToast('error', 'There is no item in the list.')
        return
      }

      const isValidCoordinate = (val) => /^-?\d+(\.\d+)?$/.test(val) && !isNaN(parseFloat(val))

      const hasInvalidItem = generatedNearbyPlacesList.some(
        (item) =>
          typeof item.location_title !== 'string' ||
          item.location_title.trim().length < 4 ||
          !item.type_match ||
          !item.distance ||
          !item.time ||
      ...
      if (hasInvalidItem) {
        addToast('error', 'One or more items have invalid or missing fields.')
        return
      }

      const result = await insertGeneratedNearbyProeprtyId(id, generatedNearbyPlacesList);
      if (result) {
        setGeneratedNearbyPlacesList([]);
        addToast('success', result.message)
        // ### update nearby state
      }
    } catch (error) {
                      ...
                      <CAccordion activeItemKey={Number(activeTab) || 1} className="mb-4">
                        ...
                        <CAccordionItem itemKey={4}>
                          ...
                          <CAccordionBody>
                            ...
                            {isEdit && activeTab === 4 && (
                              <CCol className="mb-2">
                                ...
                                <CButton
                                  className="me-2"
                                  disabled={loading}
                                  onClick={() => generateNearByLocation(propertyDetails.id)}
                                  color="primary"
                                >
                                  Genrate Here
                                </CButton> */}
                            ...
                            {generatedNearbyPlacesList.length > 0 && (
                              <>
                                <p>Here is nearby places</p>
                                <CTable>
                                  <CTableHead color="light">
                                    <CTableRow>
                                      ...
                                      <CTableHeaderCell className="w-20">
                                        Distance | Time
                                      </CTableHeaderCell>
                                      {/* <CTableHeaderCell className="w-20">Lat / Long</CTableHeaderCell> */}
                                      <CTableHeaderCell className="w-auto">_</CTableHeaderCell>
                                    </CTableRow>
                                  </CTableHead>

                                  <CTableBody>
                                    {generatedNearbyPlacesList?.map((item, index) => (
                                      <CTableRow key={index}>
                                        ...
                                        <CTableDataCell className="w-20">
                                          ...
                                          {/* <CFormInput
                                              ...
                                              className="mb-2"
                                              name={`name_${index}`}
                                              onChange={(e) => setGeneratedNearbyPlacesList(((prev, curIndex) => {
                                          ...
                                          <CFormInput
                                            value={item?.location_title || ''}
                                            type="text"
                                            className="mb-2"
                                            name={`name_${index}`}
                                            onChange={(e) =>
                                              setGeneratedNearbyPlacesList((prevList) =>
                                                prevList.map((place, i) =>
                                                  i === index
                                              ...
                                              setGeneratedNearbyPlacesList((prevList) =>
                                                prevList.map((place, i) =>
                                                  i === index
                                                    ? {
                                                        ...place,
                                                        type_match: e.target.value,
                                                        nearby_id: e.target.value,
                                                      }
                                                    : place,
                                                ),
                                              )
                                          ...
                                          >
                                            <option value={-1}>Select</option>
                                            {
                                              nearbyCategory && nearbyCategory.map((item, index) => (
                                                <option 
                                                  key={index}
                                                  value={item.id}>{item.locations_name}</option>
                                              ))
                                            }
                                          </CFormSelect>
                                        </CTableDataCell>
                                        ...
                                        {/* Distance and Time */}
                                        <CTableDataCell className="w-20 d-flex gap-2">
                                          <CFormInput
                                            className="w-20"
                                            value={item?.distance || ''}
                                            type="text"
                                        ...
                                        <CTableDataCell className="w-20 d-flex gap-2">
                                          <CFormInput
                                            ...
                                            onChange={(e) => {
                                              ...
                                              setGeneratedNearbyPlacesList((prevList) =>
                                                prevList.map((place, i) =>
                                                  i === index ? { ...place, latitude: newLat } : place,
                                                ),
                                              )
                                            }}
                                            type="text"
                                          ...
                                          <CFormInput
                                            ...
                                            onChange={(e) => {
                                              ...
                                              setGeneratedNearbyPlacesList((prevList) =>
                                                prevList.map((place, i) =>
                                                  i === index ? { ...place, longitude: newLat } : place,
                                                ),
                                              )
                                            }}
                                        ...
                                        <CTableDataCell>
                                          <FaTrash
                                            onClick={() =>
                                              setGeneratedNearbyPlacesList((prev) =>
                                ...
                                <CCol className="mb-2">
                                  <CButton
                                    className="me-2"
                                    disabled={loading}
                                    onClick={(e) => saveGeneratedLocationHandler(e)}
                                    color="primary"
                                  >
                                    Save
                                  </CButton>
                      ...
                      <CCardBody>
                        <CRow>
                          ...
                          <CCol lg="5" md="5" className="text-center border rounded p-3 m-1">
                            <a style={{ textDecoration: 'none' }} href="#intrestUsers">
                              <CHeader className="fw-bold d-flex justify-content-center align-items-center">
                          ...
                          {/* Shorlist user count */}
                          <CCol lg="5" md="5" className="text-center border rounded p-3 m-1">
                            <a style={{ textDecoration: 'none' }} href="#shorlistUsers">
                              <CHeader className="fw-bold d-flex justify-content-center align-items-center">
                          ...
                          {/* Total Lead */}
                          <CCol lg="5" md="5" className="text-center border rounded p-3 m-1">
                            <a style={{ textDecoration: 'none' }} href="#leadUsers">
                              <CHeader className="fw-bold d-flex justify-content-center align-items-center">
