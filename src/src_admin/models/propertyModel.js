...
export const generatePropertyNearbyProeprtyId = async (id = null) => {
    try {
        const result = await axiosInstance.get(`/admin/property/${id}/nearby`);
...
export const insertGeneratedNearbyProeprtyId = async (id = null, locations) => {
    try {
        const result = await axiosInstance.post(`/admin/property/${id}/nearby_locations`, {
            locations
        });
        console.log(result, "Result");
        return result.data;
